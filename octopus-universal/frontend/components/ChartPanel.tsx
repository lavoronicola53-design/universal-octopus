...
const ChartPanel = forwardRef<ChartPanelHandle, ChartPanelProps>(function ChartPanel(
  ...
  // --- overlay dell'unico scenario (proiezione Fourier) come candele future ---
  useEffect(() => {
    ...
    const dominant = scenarios.find((s) => s.dominant) ?? scenarios[0];
    const projCandles = (dominant?.candles ?? []).map((c) => ({
      time: toChartTime(c.timestamp), open: c.open, high: c.high, low: c.low, close: c.close,
    }));
    dominantSeriesRef.current.setData(projCandles);

    // eventuali scenari secondari (solo in modalita' avanzata) come linee
    ...
    secondary.forEach((s, i) => {
      const line = chartRef.current!.addLineSeries({
        ...
        title: `proiezione ${i + 2}`,
      });
      line.setData(s.candles.map((c) => ({ time: toChartTime(c.timestamp), value: c.close })));
      lineSeriesRefs.current.push(line);
    });

    // adatta la vista temporale per includere anche le candele proiettate,
    // cosi' la proiezione e' sempre visibile subito dopo il calcolo
    if (projCandles.length > 0) {
      chartRef.current.timeScale().fitContent();
    }
  }, [scenarios]);
